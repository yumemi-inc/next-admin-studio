---
name: 'form-input-date-time'
root: './src/'
output: '/model/**/components/form-with-preview'
ignore: []
questions:
  property: 'プロパティ名を入力してください (eg. date)'
  required:
    confirm: '公開時 必須項目ですか？'
    initial: true
---
...
export const {{ name | snake | upper }}_DEFAULT_VALUE: Date | null = null;
```
...
export type {{ model | pascal }}{{ inputs.property | pascal }}Slice = {
  {{ inputs.property | camel }}: Date | null;
  set{{ inputs.property | pascal }}: ({{ inputs.property | camel }}: Date | null) => void;
...
import {
  notEmptyDateValidation,
} from "@/common/lib/form-validation/date-input";
...
export const {{ model | pascal }}{{ inputs.property | pascal }}PreviewView = ({ value }: { value: Date | null }) => {

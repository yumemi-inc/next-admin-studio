---
name: 'form-input-radio-button-group'
root: './src/'
output: '/model/**/components/form-with-preview'
ignore: []
questions:
  property: 'プロパティ名を入力してください (eg. store)'
  required:
    confirm: '公開時 必須項目ですか？'
    initial: true
---
...
export const {{ name | snake | upper }}_DEFAULT_VALUE: string = "";
```
...
import { notEmptyRadioButtonGroupInputValidation } from "@/common/lib/form-validation/radio-button-group-input";
...
export const {{ model | camel }}{{ inputs.property | pascal }}Validation = (v: string): InputValidation => ({
  onConfirmedSubmit: [notEmptyRadioButtonGroupInputValidation(v)],
});
